...
const SECTION_PERSONAL: TemplateSection = {
  ...
  columns: [
    ...
    { header: 'Birthdate (YYYY-MM-DD)', width: 20, example: '1996-05-20', required: true },
...
// Coerce a cell into a YYYY-MM-DD string, or "" if it isn't a parseable date.
...
async function rowToPayload(row: Row, services: string[], caches: ResolveCaches): Promise<Record<string, unknown>> {
  ...
  const birthdateRaw = get(row, 'Birthdate (YYYY-MM-DD)')
  if (!birthdateRaw) throw new Error('Birthdate is required.')
  const birthdate = toIsoDate(birthdateRaw)
  if (!birthdate) throw new Error(`Birthdate "${birthdateRaw}" is not a valid date — use YYYY-MM-DD format (e.g. 1996-05-20).`)
  ...
  return {
    ...
    serviceAvailed,
    dateApplicationReceived: toIsoDate(get(row, 'Date Applied (YYYY-MM-DD)')),

...
interface AddressFieldsProps {
  value: AddressValue
  onChange: (addr: Partial<AddressValue>) => void
  inputClass: string
  labelClass: string
}

// ─── Static PH address data ────────────────────────────────────────────────────
...
const PH_DATA: Record<string, Record<string, string[]>> = {
  ...
  'Region IV-B – MIMAROPA': {
    ...
    'Palawan': ["Aborlan", "Agutaya", "Araceli", "Balabac", "Bataraza", "Brooke\'s Point", "Buliluyan", "Cagayancillo", "Coron", "Culion", "Cuyo", "Dumaran", "El Nido", "Española", "Kalayaan", "Linapacan", "Magsaysay", "Narra", "Puerto Princesa City", "Quezon", "Rizal", "Roxas", "San Vicente", "Sofronio Española", "Taytay"],
  ...
  'Region VI – Western Visayas': {
    'Aklan': ['Altavas', 'Balete', 'Banga', 'Batan', 'Buruanga', 'Ibajay', 'Kalibo', 'Lezo', 'Libacao', 'Madalag', 'Malay', 'Malinao', 'Nabas', 'New Washington', 'Numancia', 'Tangalan'],
  ...
  'Region IX – Zamboanga Peninsula': {
    'Zamboanga del Norte': ['Baliguian', 'Dapitan City', 'Dipolog City', 'Godod', 'Gutalac', 'Jose Dalman', 'Kalawit', 'Katipunan', 'La Libertad', 'Labason', 'Leon B. Postigo', 'Liloy', 'Manukan', 'Mutia', 'Piñan', 'Polanco', 'Rizal', 'Roxas', 'Salug', 'San Miguel', 'Sergio Osmeña Sr.', 'Siayan', 'Sibuco', 'Sibutad', 'Sindangan', 'Siocon', 'Sirawai', 'Tampilisan'],
    'Zamboanga del Sur': ['Aurora', 'Bayog', 'Dimataling', 'Dinas', 'Dumalinao', 'Dumingag', 'Guipos', 'Josefina', 'Kumalarang', 'Labangan', 'Lakewood', 'Lapuyan', 'Mahayag', 'Margosatubig', 'Midsalip', 'Molave', 'Pagadian City', 'Pitogo', 'Ramon Magsaysay', 'San Miguel', 'San Pablo', 'Sominot', 'Tabina', 'Tukuran', 'Vincenzo A. Sagun', 'Zamboanga City'],
  ...
  'Region X – Northern Mindanao': {
    ...
    'Lanao del Norte': ['Bacolod', 'Baloi', 'Baroy', 'Buru-un', 'Iligan City', 'Kapatagan', 'Kauswagan', 'Kolambugan', 'Lala', 'Linamon', 'Magsaysay', 'Maigo', 'Matungao', 'Munai', 'Nunungan', 'Pantao Ragat', 'Pantar', 'Poona Piagapo', 'Salvador', 'Sapad', 'Sultan Naga Dimaporo', 'Tagoloan', 'Tangcal', 'Tubod'],
    'Misamis Occidental': [
  ...
  'Region XII – SOCCSKSARGEN': {
    'Cotabato (North Cotabato)': ['Alamada', 'Aleosan', 'Antipas', 'Arakan', 'Banisilan', 'Carmen', 'Kabacan', 'Kidapawan City', 'Libungan', 'Magpet', 'Makilala', 'Matalam', 'Midsayap', 'M\'lang', 'Pigkawayan', 'Pikit', 'President Roxas', 'Tulunan'],
  ...
  'BARMM – Bangsamoro Autonomous Region': {
    ...
    'Lanao del Sur': ['Amai Manabilang', 'Bacolod-Kalawi', 'Balabagan', 'Balindong', 'Bayang', 'Binidayan', 'Buadiposo-Buntong', 'Bubong', 'Bumbaran', 'Butig', 'Calanogas', 'Ditsaan-Ramain', 'Ganassi', 'Kapai', 'Kapatagan', 'Lumba-Bayabao', 'Lumbaca-Unayan', 'Lumbatan', 'Lumbayanague', 'Madalum', 'Madamba', 'Maguing', 'Malabang', 'Marantao', 'Marawi City', 'Marogong', 'Masiu', 'Molundo', 'Mulondo', 'Pagayawan', 'Piagapo', 'Poona Bayabao', 'Pualas', 'Saguiaran', 'Sultan Dumalondong', 'Sultan Gumander', 'Tagoloan II', 'Tamparan', 'Taraka', 'Tubaran', 'Tugaya', 'Wao'],
    ...
    'Maguindanao del Sur': ['Ampatuan', 'Buluan', 'Datu Abdullah Sangki', 'Datu Anggal Midtimbang', 'Datu Hoffer Ampatuan', 'Datu Piang', 'Datu Salibo', 'Datu Saudi-Ampatuan', 'Datu Unsay', 'General Salipada K. Pendatun', 'Guindulungan', 'Mamasapano', 'Mangudadatu', 'Pagalungan', 'Paglat', 'Pandag', 'Rajah Buayan', 'Shariff Aguak', 'Shariff Saydona Mustapha', 'South Upi', 'Sultan sa Barongis', 'Talayan', 'Talitay'],
...
const TANGUB_CITY_BARANGAYS = [
  'Baybay', 'Bocator', 'Bongbong', 'Bongon', 'Calubian', 'Camanga', 'Capalaran',
  'Cub-cub', 'Dalipe', 'Datagan', 'Hubangon', 'Lao-ang', 'Lapasan', 'Lusot',
  'Maloro', 'Mantic', 'Migcanaway', 'Minsubong', 'Misom', 'Mosangkong',
  'Pangapason', 'Panalsalan', 'Poblacion I', 'Poblacion II', 'Poblacion III',
  'Punta', 'Ronco', 'Sambulawan', 'Santa Cruz', 'Sinonok', 'Sta. Clara',
  'Taboc', 'Taguite', 'Tigbao', 'Titugan', 'Tubod', 'Villanueva', 'Caniangan',
]

export default function AddressFields({ value, onChange, inputClass, labelClass }: AddressFieldsProps) {
  const regions = Object.keys(PH_DATA)
  const provinces = value.region ? Object.keys(PH_DATA[value.region] ?? {}) : []
  const cities = value.province && value.region ? (PH_DATA[value.region]?.[value.province] ?? []) : []
  const barangays = value.cityMunicipality === 'Tangub City' ? TANGUB_CITY_BARANGAYS : []

  const sel = `${inputClass} bg-white`

  const handleRegionChange = (region: string) => {
    onChange({ region, province: '', cityMunicipality: '', barangay: '', streetPurok: value.streetPurok })
  }
  const handleProvinceChange = (province: string) => {
    onChange({ province, cityMunicipality: '', barangay: '' })
  }
  const handleCityChange = (city: string) => {
    onChange({ cityMunicipality: city, barangay: '' })
  }
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        ...
        <select className={sel} value={value.region} onChange={(e) => handleRegionChange(e.target.value)}>
      ...
      <div>
        ...
        >
          <option value="">{value.region ? 'Select province' : 'Select a region first'}</option>
      ...
      <div>
        ...
        >
          <option value="">{value.province ? 'Select city / municipality' : 'Select a province first'}</option>
      ...
      <div>
        ...
        {barangays.length > 0 ? (
          <select
            ...
            disabled={!value.cityMunicipality}
          >
            <option value="">{value.cityMunicipality ? 'Select barangay' : 'Select a city / municipality first'}</option>
          ...
          <input
            ...
            value={value.barangay}
            onChange={(e) => onChange({ barangay: e.target.value })}
            placeholder={value.cityMunicipality ? 'Enter barangay' : 'Select a city / municipality first'}
            disabled={!value.cityMunicipality}
          />
        )}
      </div>
      <div className="col-span-2">
        ...
        <input
          ...
          onChange={(e) => onChange({ streetPurok: e.target.value })}

// Bulk applicant import for DILP.
//
// Two responsibilities:
//   1. Generate a styled .xlsx template (ExcelJS) that mirrors the DILP Profile
//      form section by section, matching the look of the CDSP/SLP/CLPEP/Skills
//      Training import templates: a merged, colored section-label header row
//      (row 1) above the individual field headers (row 2), with enum fields
//      rendered as in-cell dropdowns.
...
// Beneficiary Classification, 4Ps Beneficiary?, and Year Graduated / Exited all
// live inside Section I (Personal Information) in the real DILPProfileForm.tsx
// (Step 1) -- not their own sections -- so this mirrors that exactly rather
// than inventing extra sections the on-screen form doesn't have.
const SECTION_PERSONAL: TemplateSection = {
  ...
  columns: [
    ...
    { header: 'Email Address', width: 22, example: '' },
    { header: 'Beneficiary Classification', width: 24, example: 'Vendor', options: CLASSIFICATION_OPTIONS },
    { header: 'Classification, if Others', width: 24, example: '' },
    { header: '4Ps Beneficiary?', width: 16, example: 'No', options: YES_NO_OPTIONS },
    { header: 'Year Graduated / Exited', width: 20, example: '' },
  ],
}
...
export async function downloadImportTemplate(): Promise<void> {
  ...
  link.href = url
  link.download = 'DILP_Beneficiaries_Template.xlsx'
  link.click()
...
async function rowToPayload(row: Row, caches: ResolveCaches): Promise<Record<string, unknown>> {
  ...
  const is4PsRaw = pickEnum(get(row, '4Ps Beneficiary?'), YES_NO_OPTIONS)
  const is4PsBeneficiary = is4PsRaw === 'Yes'
  ...
  return {
    ...
    beneficiaryClassification: pickEnum(get(row, 'Beneficiary Classification'), CLASSIFICATION_OPTIONS),
    beneficiaryClassificationOther: get(row, 'Classification, if Others'),
    is4PsBeneficiary,
    yearGraduated4Ps: is4PsBeneficiary ? get(row, 'Year Graduated / Exited') : '',
...
export async function importDilpApplicants(
  file: File,

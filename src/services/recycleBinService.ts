...
export type RecycleRecordType = 'applicant' | 'employer' | 'referral' | 'gipApplicant' | 'cdspApplicant' | 'spesApplicant' | 'dilpApplicant' | 'tupadApplicant'
...
export async function listDeleted(): Promise<RecycleBinRecord[]> {
  const [efRes, gipRes, cdspRes, spesRes, dilpRes, tupadRes] = await Promise.all([

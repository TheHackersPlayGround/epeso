        ...
        <div className="flex flex-col items-center text-center mb-6">
          {type === 'success' ? (
            ...
            <div className="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center mb-4">
              <svg className="w-8 h-8 text-[#0077BE]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-gray-500 text-base mt-2 whitespace-pre-line">{message}</p>
        </div>
        <div className="flex gap-3">
          {type === 'confirm' && cancelText && (
            <button
              onClick={onCancel}
              className="flex-1 py-2.5 border border-gray-300 rounded-lg text-base text-gray-600 hover:bg-gray-50 transition-colors"
            >
              {cancelText}
            </button>
          )}
          <button
            onClick={onConfirm}
            className={`flex-1 py-2.5 rounded-lg text-base text-white font-medium transition-colors ${

    ...
    <div className="flex flex-wrap gap-3">
      <div className="relative flex-1 min-w-48">
        ...
        <input
          ...
          placeholder="Search CLPEP beneficiaries..."
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-brand-blue">
            ...
            <th className="px-4 py-3 text-left text-white font-semibold whitespace-nowrap">Cooperative Name</th>
            <th className="px-4 py-3 text-left text-white font-semibold whitespace-nowrap">Role</th>
          ...
          {beneficiaries.length === 0 ? (
            ...
            beneficiaries.map(b => (
              <tr key={b.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{b.cooperativeName ?? '-'}</td>
                <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{b.cooperativeRole ?? '-'}</td>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
        ...
        <div className="px-6 py-5">
          <div className="grid grid-cols-2 gap-x-8 gap-y-4">
            ...
            <div>
              <p className="text-xs text-gray-500 mb-0.5">Cooperative Name</p>
              <p className="text-sm font-medium text-gray-800">{beneficiary.cooperativeName ?? '-'}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 mb-0.5">Role</p>
              <p className="text-sm font-medium text-gray-800">{beneficiary.cooperativeRole ?? '-'}</p>
            </div>
            <div>
...
type FormModalProps = {
  initial: CLPEPFormData
  title: string
  onClose: () => void
  onSave: (data: CLPEPFormData) => void
}

function FormModal({ initial, title, onClose, onSave }: FormModalProps) {
  const [form, setForm] = useState<CLPEPFormData>(initial)
  const [showErrors, setShowErrors] = useState(false)

  function handleFieldChange(field: keyof CLPEPFormData, value: string) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-5 grid grid-cols-2 gap-x-6 gap-y-4 overflow-y-auto">
          ...
          <div className="col-span-2">
            <label htmlFor="clpep-name" className="block text-sm font-medium text-gray-600 mb-1">
          ...
          {/* Contact Number */}
          <div>
            <label htmlFor="clpep-contact" className="block text-sm font-medium text-gray-600 mb-1">
              Contact Number
            </label>
            <input
              id="clpep-contact"
          ...
          {/* Barangay */}
          <div>
            <label htmlFor="clpep-barangay" className="block text-sm font-medium text-gray-600 mb-1">
              Barangay
            </label>
            <input
              id="clpep-barangay"
          ...
          {/* Date Enrolled */}
          <div>
            <label htmlFor="clpep-date-enrolled" className="block text-sm font-medium text-gray-600 mb-1">
              Date Enrolled <span className="text-red-500">*</span>
            </label>
            <input
              id="clpep-date-enrolled"
              type="date"
          ...
          {/* Cooperative Name */}
          <div>
            <label htmlFor="clpep-coop-name" className="block text-sm font-medium text-gray-600 mb-1">
              Cooperative Name
            </label>
            <input
              id="clpep-coop-name"
              ...
              onChange={e => handleFieldChange('cooperativeName', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none text-gray-900"
            />
          </div>
          ...
          {/* Cooperative Role */}
          <div>
            <label htmlFor="clpep-coop-role" className="block text-sm font-medium text-gray-600 mb-1">
              Role in Cooperative
            </label>
            <input
              id="clpep-coop-role"
              ...
              value={form.cooperativeRole}
              onChange={e => handleFieldChange('cooperativeRole', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none text-gray-900"
            />
          </div>
          ...
          {/* Status */}
          <div>
            <label htmlFor="clpep-status" className="block text-sm font-medium text-gray-600 mb-1">
              Status
            </label>
            <select
              id="clpep-status"
...
export default function CLPEPTab() {
  ...
  function handleAddBeneficiary(data: CLPEPFormData) {
  ...
  function handleEditSave(data: CLPEPFormData) {
  ...
  function buildExportRows() {
    return filtered.map(b => ({
      ...
      'Cooperative Name': b.cooperativeName ?? '',
      'Role': b.cooperativeRole ?? '',
      'Status': b.status,
    }))
  }
      ...
      {editingBeneficiary && (
        <FormModal
          initial={{
            ...
            cooperativeName: editingBeneficiary.cooperativeName ?? '',
            cooperativeRole: editingBeneficiary.cooperativeRole ?? '',
            status: editingBeneficiary.status,
          }}
          title="Edit CLPEP Beneficiary"
